...
const Food = (props) => {
  ...
  const handleRemoveFromCartClick2 = () => {
    axios
      .get(
        "http://188.121.124.63/restaurant/restaurant_view/" +
          resid +
          "/" +
          userid +
          "/order/remove_from_order/" +
          food.id +
          "/",
        {
          headers: {
  ...
  React.useEffect(() => {
    axios
      .get("http://188.121.124.63/restaurant/restaurant_view/" + resid + "/", {
        headers: {
          "Content-Type": "application/json",
          "Access-Control-Allow-Origin": "*",
          "Access-Control-Allow-Methods": "PUT,PATCH",
      ...
      <Card sx={{ borderRadius: 2 }} className="card-food">
        <CardActionArea>
          ...
          <CardContent sx={{ height: 25 }} className="food-animation">
            ...
            <Typography variant="body2" color="#ffa600">
              remaining: {remainder}
            </Typography>
          </CardContent>
        </CardActionArea>
        <CardActions>
          <Grid container spacing={3.5} className="food-card-grid">
            ...
            <Grid item lg={5} md={5} sm={5} className="count-buttons">
              ...
              <h5 className="food-h5" onChange={handleChange}>
                {count}
              </h5>
              <button
                className={`button__wrapper ${
